...
g.newViewEvent=function(c,a,b){if(r(c)){for(var d in c)g.newViewEvent(d,c[d][0],c[d][1]);return this}var f=function(a){(!0===b||b(a))&&i(a.target).trigger(k({},a,{type:c,currentTarget:a.target}))};if(i.event.special[c])throw"event '"+c+"' has been defined";i.event.special[c]={setup:function(){i(this).bind(a,f)},teardown:function(){i(this).unbind(a,f)}};return this};var Pb=i.cleanData;i.cleanData=function(c){i(c).each(function(){Wa(this)});Pb(c)};G.getUniqueViewEventTypes=Ra;G._viewHandlerGateway=
...
d.by=f[0];d.asc=!!f[1]},get:function(a){a=a.workflow;this.sort(a.by,a.asc);a.asc=!a.asc}}],clear:["$click:.|*clear",function(){this.clear()}],del:["$click:.|*del",function(){this.del()}]},sa;for(sa in jb){var kb=jb[sa];g.groups[sa]=kb[0];g.workflowType(sa,kb[1])}k(g.groups,{caption:function(a,b,d,f){i(a).prepend("<option value=''>"+(f||g.get(b))+"</option>")},autofocus:function(a){setTimeout(function(){i(a).focus()},1)},mapEvent:function(a,b,d,f){f=f.split(",");i(a).mapEvent(f[0],f[1],f[2])},mapClick:function(a,
b,d,f){f=f.split(",");i(a).mapEvent("click",f[0],f[1])},logPanel:function(a,b,d){var a=i(a),f=a.is("ol")?a:i("<ol style='font-family: monospace, serif' />").appendTo(a),a=f[0];d.appendSub(a,"*log","init",function(a){for(var a=a.publisher.get(),b=0;b<a.length;b++)f.append("<li>"+a[b]+"</li>")});d.appendSub(a,"*log","afterCreate.1",function(a){f.append("<li>"+a.originalPublisher.raw()+"</li>")});d.appendSub(a,"*log","afterCreate",function(){f.empty()})},clearlog:"clear:/*log",enableLater:"!after*:.|*enable",
...
warn:"!after*:*errors|*warn",warnSummary:"!afterUpdate* validityChecked:.|*renderErrorSummary;!validityReset:.|empty"});g.subscription.special.validityChanged={setup:function(a,b){var d=b+"*isValid";j(g.get(d))&&g.sub(b,"*invalidPaths","!after*. after*.1",function(){var a=ab(b);if(g.get(d)!==a){g.trigger(b,b,"validityChanged",a,!a);g.set(d,a)}})}};k(F,{checkValidity:function(a){a=this.getPath(a);db(a,function(a){o(a,a,"checkValidity",n.get(a))});var b=ab(a);g.trigger(a,a,"validityChecked",b);return b},
...
}},{name:"fixedValue",error:'Please enter value "{fixedValue}"',isValid:function(a,b){return a==b.fixedValue},initialize:function(a,b,d,f){if(m(f)){if(a=/^(\w+)(,(.*))*$/.exec(f))d.options={fixedValue:ea(a[1]),error:a[3]}}else if(r(f))d.options=f;else throw"missing options in fixedValue validator";},buildError:M}]);var wb=g.workflowType("v_required").get;"undefined"!=typeof matrix&&(B.load=function(a){return matrix(a+".template")},matrix.loader.set("template",{load:{compile:function(a,b){i(b).filter("script[id]").each(function(){var a=
i(this);B.compile(this.id,a.html(),a.attr("type")||B.defaultEngine)})},buildDependencies:"parseDependsTag"},url:function(a){return B.templateIdToUrl(matrix.fileName(a))}}),B.templateIdToUrl=function(a){var b;if(a.startsWith("*")){a=a.substr(1);b=a.split(".");a="_";b=b[0]}else{b=a.split(".");a=b[0];b=b[1]||"main"}return"template/"+a+"/"+b+".html"});g.workflowType({addRowView:X("*getOriginal",function(a,b){var d=this.children();if(d.length===0)this.append(a);else{var f=+b.originalPublisher.pathIndex();
f===0?d.eq(0).before(a):d.eq(f-1).after(a)}}),updateRowView:X("*getOriginal",function(a,b){this.children().eq(+b.originalPublisher.pathIndex()).replaceWith(a)}),removeRowView:function(a){this.children().eq(+a.originalPublisher.pathIndex()).remove()}});F.initQueryable=function(a){function b(){var a=j.ops,b=j.by,f=j.value,h;if(f){if(a)if(a=="equals")h=y("^"+f+"$","i");else throw"operator does not supported";else h=y(f,"i");g=b?function(){return h.test(this[b])}:function(){if(r(this)){for(var a in this)if(h.test(this[a]))return true;
return false}return h.test(this)};q.set(true);d.refreshQuery()}else if(g){g=null;q.set(false);d.refreshQuery()}}if(this.get("*query"))return this;var d,f,h,g,j,k=this.get(),l=this.cd("*query"),m=this.cd("*queryResult"),n=this.cd("*hasQueryResult"),p=l.cd("pager"),o=l.cd("filter"),q=o.cd("enabled"),s=l.cd("sort");(a=!!a)&&m.reference(l.path);m.reference(this.path);this.extend("*",d={autoQuery:a,hasQueryResult:false,query:{pager:h={enabled:false,index:0,count:1,size:0},sort:f={by:null,asc:null},filter:j=
{by:"",value:"",ops:"",enabled:false},enabled:function(){return this.get("pager.enabled")||this.get("sort.by")||this.get("filter.enabled")}},queryResult:function(a){var b=i(k),b=g?b.filter(g).get():b.get();n.update(b.length>0);f.by&&b.sortObject(f.by,f.asc);if(!a&&h.enabled){a=Math.ceil(b.length/h.size)||1;if(a!=h.count){h.count=a;if(h.index>h.count-1)h.index=0;l.triggerChange("pager")}b=b.slice(h.index*h.size,(h.index+1)*h.size)}return b},refreshQuery:function(){d.autoQuery||setTimeout(function(){m.trigger("afterUpdate")},
0)},paging:function(a){a=a.eventData;if(lb.test(a))a=+a;else if(a=="next")a=h.index+1;else if(a=="previous")a=h.index-1;else if(a=="first")a=0;else if(a=="last")a=h.count-1;else if(a=="disabled"){a=0;d.resetPaging()}if(typeof a!=="number"||a<0||a>h.count-1)a=0;p.update("index",a);d.refreshQuery()},resetSort:function(a){s.set("by",null).set("asc",null);a!==true&&d.refreshQuery()},resetSearch:function(a){o.update("by","").update("value","").update("ops","");a!==true&&d.refreshQuery()},resetPaging:function(a){p.update("enabled",
...
b==-1?"new":"update"}};k(F,{initShadowEdit:function(a){var b=this;if(!b.get("*edit")){var d=b.path,f=y("^"+d.replace(".","\\.")+"\\*edit\\.item[^*]+\\*$"),h=y("^"+d.replace(".","\\.")+"\\*edit\\.item[\\w.]+\\*edit\\.item");if(j(a))if(b.isShadow()&&!p(b.raw()))var a=b.main(),g=(a.logicalPath()+"*edit.item").length+1,d=C(d).substr(g),a=aa(a.get("*edit.itemTemplate."+d+".0"),true);else{a=p(b.raw())?n.raw(b.main().path+"_itemTemplate"):n.raw(d+"_itemTemplate");j(a)&&(a=Fa(aa(b.get()[0],true)))}d=new Ba(-1);
...
b,d){d.appendSub(a,b+"*edit.mode","init afterUpdate",function(a){a=a.publisher.get();this[j(a)||a=="read"?"hide":"show"]()})},hideOnEdit:function(a,b,d){d.appendSub(a,b+"*edit.mode","init afterUpdate",function(a){a=a.publisher.get();this[j(a)||a=="read"?"show":"hide"]()})},editRow:"mapClick:_|edit",deleteRow:"confirm:_|Do you want to delete this item?;mapClick:_|delete;",newItem:"$click:.|*newShadowItem;hideOnNew:.",editObject:"$click:.|*editShadowItem;hideOnEdit:.",moveUpButton:"mapClick:_|moveUp",
moveDownButton:"mapClick:_|moveDown",saveButton:"$click:.|*saveShadowItem",cancelSaveButton:"$click:.|*resetShadowItem"});if(i.bbq){var ta=[],nb=i.bbq.getState,Yb=i.bbq.pushState,Zb=function(a){var a=r(a)?C(a.publisher.path):a,b={};b[a]=n.get(a);Yb(b)};g.bookmarkable=function(){var a,b,d=nb(),f={};for(a=0;a<arguments.length;a++){b=arguments[a];b in d&&n.set(b,d[b]);g.sub(null,b,"afterUpdate",Zb);ta.push(b)}for(a=0;a<ta.length;a++){b=C(ta[a]);f[b]=n.get(b)}a=i.param.fragment(location.href,f);history.replaceState?
...
l.selectedClass;a.publisher.get()==b?this.addClass(d):this.removeClass(d)},highlightTabInContainer:function(a){var b=a.publisher.get(),d=l.tabViewAttr,f=l.tabLinkAttr,a=a.workflow.options,g=a.selectedClass||l.selectedClass;this.find(a.selector).andSelf().each(function(a,j){var k=i(j);(k.attr(d)||k.attr(f))==b?k.addClass(g):k.removeClass(g)})}});g.groups.tab=function(a,b,d,f){d.appendSub(a,b,"init afterUpdate","*highlightTab",f);i(a).attr(l.tabLinkAttr)&&d.appendSub(b,a,"click",eb,l.tabLinkAttr)};
g.groups.tabContainer=function(a,b,d,f){var f=(f||"").split(","),g=l.tabLinkAttr,i=l.tabGroupAttr,j=f[0]?"["+i+"='"+f[0]+"']":"",i="["+g+"]"+j,j=i+",["+l.tabViewAttr+"]"+j;d.appendSub(b,a,"click",eb,g,i);d.appendSub(a,b,"init100 afterUpdate","*highlightTabInContainer",{selector:j,selectedClass:f[1]})};var P={},$b=/^([^,]+)(,(.+))?$/,ob=/^([^,]+)(,([^,]+))?(,(.+))?$/;g.App=g.Class.extend({bootstrap:function(a,b,d){if(a&&this.loadable()){var f=this,g,j=f.name;if(!ga.get(a,j)){f.buildRootModel!==false&&
(g=f.buildRootModel(b,d));N(g)?g.done(function(){f.buildRootView(a,b)}):f.buildRootView(a,b);ga.add(a,b,j,f);i(a).bind("shutdown."+j,function(a){f.shutdown(this,a.namespace);a.stopPropagation()});f.instanceCount++}}},shutdown:function(a,b){var d=this.name;this.destroyRootView(a);this.destroyRootModel(b);ga.remove(a,d);i(a).unbind("shutdown."+d);this.instanceCount--},fetchRootData:null,buildRootModel:function(a,b){var d=this;if(!d.fetchRootData)throw"app.fetchRootData is not implemented";var f=d.fetchRootData(a,
b);if(N(f))return f.done(function(b){g.set(d.getNamespace(a),b)});g.set(d.getNamespace(a),f)},destroyRootModel:function(a){this.buildRootModel!==false&&g(a).del(this.name)},buildRootView:function(a,b){i(a).renderContent(this.getTemplateOptions(),this.getNamespace(b))},destroyRootView:function(a){i(a).empty()},getTemplateOptions:function(){return this.templateOptions||this.name},getNamespace:function(a){return g(a).getPath(this.subPath||this.name)},initialize:function(a){if(!a.name)throw"An app must have a name.";
...
this.fetch(a).done(function(){P[a].bootstrap(b,d,f)})}},shutdown:function(a,b){if(j(a)){a=b;b=document.body}var d=ga.get(b,a);d&&d.app.shutdown(b,d.modelContainer)}});var ga={get:function(a,b){return this.appData(a)[b]},add:function(a,b,d,f){this.appData(a)[d]={app:f,modelContainer:b}},remove:function(a,b){delete this.appData(a)[b]},appData:function(a,b){var d=i(a).hmData("app");if(!b&&!d){d={};i(a).hmData("app",d)}return d}};k(g.groups,{app:function(a,b,d,f){d=$b.exec(i.trim(f));g.App.bootstrap(d[1],
a,b,d[3])},bootstrap:function(a,b,d,f){var d=ob.exec(i.trim(f)),h=d[1],j=i(d[3])[0],k=d[5];i(a).click(function(){g.App.bootstrap(h,j,b,k)})},shutdown:function(a,b,d,f){var b=ob.exec(i.trim(f)),h=b[1],j=i(b[3])[0];i(a).click(function(){g.App.shutdown(h,j)})},selfShutdown:function(a,b,d,f){i(a).mapEvent("click","shutdown."+f)}});var ac=i.cleanData;i.cleanData=function(a){i(a).each(function(){var a=ga.appData(this,true);if(a)for(var d in a){var f=a[d];delete a[d];f.unload(this,true)}});ac(a)};"undefined"!==
