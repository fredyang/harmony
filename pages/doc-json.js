[
    ...
    },
    {
        ...
        "namespace": "",
        "shortDesc": "Core",
        "longDesc": "<p>This section is the reference the source in harmony.core.js. In the documents here, When we refer the model, it means repository node or model node. When we refer to view, it means jQuery object or DOM element.\n</p>",
        "signatures": []
    },
    {
        "name": "Model",
    ...
    },
    {
        ...
        "namespace": "Core",
        "shortDesc": "Template",
        "longDesc": "<p>Template module include several activities, workflow types, subscription group, and template engine adapter.</p>\n\n<p>In Harmony, a complex view is rendered by template. And this process is handled by template workflow which is an instance of workflow type. All template workflow type use three special activities, initialize activity <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.templateOptions\">templateOptions</a>, convert activity <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.dataToMarkup\">dataToMarkup</a>, and finalize activity <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.parseSub\">parseSub</a>.</p>",
        "signatures": []
    },
    {
        "name": "get",
        "type": "",
        "namespace": "Core.Model.Node.Manipulation",
        "shortDesc": "node.get()",
    ...
    },
    {
        "name": "pushStack",
        "type": "",
        "namespace": "Core.Model.Node.Navigation",
    ...
    },
    {
        ...
        "longDesc": "<p>In repository each node (main node) can have a shadow node.  They are normally use in <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=plugins\">plugins development</a>.\n</p>",
        "signatures": []
    },
    {
        "name": "cd",
    ...
    },
    {
        "name": "indexOf",
        "type": "",
        "namespace": "Core.Model.Node.Array methods",
    ...
    },
    {
        "name": "isEmpty",
        "type": "",
        "namespace": "Core.Model.Node.Utilities",
    ...
    },
    {
        "name": "saveLocal",
        "type": "",
        "namespace": "Core.Model.Node.LocalStorage",
    ...
    },
    {
        "name": "Event Bubbling",
        "type": "",
        "namespace": "Core.Unified Subscription.Model Event",
    ...
    },
    {
        ...
        "shortDesc": "object handler",
        "longDesc": "<p>Each workflow object can have at most five activity functions:  \"initialize\", \"get\", \"set\", \"convert\", \"finalize\".  The \"initialize\" activity will be executed once when subscription is called. The other four activities are executed in sequence when event is triggered. The result of one activity will be passed along to next activity. The following examples shows how to use workflow object as handler in a subscription. The two label subscribe the events of a button using the same workflow object. This object has all four activities. \n</p>\n\n<pre data-sub=\"prettyprint:_|;preview:_|http://jsbin.com/omidom/1\">\n\nvar workflowObject = {\n\n  initialize: function (publisher, subscriber, workflow, options) {\n    workflow.started = new Date();\n    workflow.unit = options;\n    hm.log(\"subscription created for: \" + subscriber[0].id, \"red\");\n  },\n\n  get: function (e) {\n    return new Date();\n  },\n\n  convert: function (value, e) {\n\n    var now = value.getHours() + \":\" + \n        value.getMinutes() + \":\" + \n        value.getSeconds() + \".\" + \n        value.getMilliseconds();\n\n    var duration = (value - e.workflow.started) / 1000;\n\n    if (e.workflow.unit == \"second\") {\n\n      return \"Now: \" + now +\n        \", Time elapsed since subscription: \" + \n        ((value - e.workflow.started) / 1000) +\n        \" seconds has passed\";\n\n    } else {\n\n      return \"Now: \" + now +\n        \", Time elapsed since subscription: \" + \n        (value - e.workflow.started) +\n        \" mini-seconds has passed\";\n    }\n  },\n\n  set: function (value, e) {\n    this.text(value);\n\n  },\n\n  finalize: function (value, e) {\n\n    hm.log(\n      \"finalized for subscriber: \" + this[0].id +\n      \", publisher: \" + e.publisher[0].id +\n      \", event: \" + e.type);\n  }\n\n};\n\n$(\"#lableOne\").sub(\"$#buttonOne\", \"click\", workflowObject, \"second\");\n\nsetTimeout(function () {\n  $(\"#lableTwo\").sub(\"$#buttonOne\", \"click\", workflowObject);\n}, 1000);\n</pre>",
        "signatures": []
    },
    {
        "name": "function",
        "type": "",
        "namespace": "Core.Unified Subscription.Subscription.Handler",
        "shortDesc": "function handler",
        "longDesc": "<p>If the processing logics can not be reused, instead of splitting the workflow into reusable activity,  you can put all the logics into a single function,  and use this function as the handler. Internally, this function handler will be converted to workflow object with just a \"get\" activity which points to this function handler.</p>\n\n<pre data-sub=\"prettyprint:_;preview:_|http://jsbin.com/ukejag/1\">\n\n\nvar functionHandler = function (e) {\n  var value = new Date();\n  var now = value.getHours() + \":\" +\n         value.getMinutes() + \":\" + \n         value.getSeconds() + \".\" +\n         value.getMilliseconds();\n\n  var options = e.workflow.options;\n\n  var duration = (value - options.started) / 1000;\n  var message;\n\n  if (options.unit == \"second\") {\n\n    message = \"Now: \" + now +\n      \", Time elapsed since subscription: \" + \n      ((value - options.started) / 1000) +\n      \" seconds has passed\";\n\n  } else {\n\n    message = \"Now: \" + now +\n      \", Time elapsed since subscription: \" + \n      (value - options.started) +\n      \" mini-seconds has passed\";\n  }\n  this.text(message);\n};\n\n$(\"#lableOne\").sub(\"$#buttonOne\", \"click\", functionHandler, {\n  started: new Date(),\n  unit: \"second\"\n});\n\nsetTimeout(function () {\n\n  $(\"#lableTwo\").sub(\"$#buttonOne\", \"click\", functionHandler, {\n    started: new Date()\n  });\n}, 1000);\n</pre>",
        "signatures": []
    },
    {
        "name": "string",
        "type": "",
        "namespace": "Core.Unified Subscription.Subscription.Handler",
        "shortDesc": "string handler",
        "longDesc": "<p>There are three formats of string-formed handler: activity expression,  node path expression, and workflow type expression.</p>",
        "signatures": []
    },
    {
        "name": "empty",
        "namespace": "Core.Unified Subscription.Subscription.Handler",
        "shortDesc": "empty handler",
        "longDesc": "<p>\nIn string-formed handler, we can embed a handler into the repository, use a \"path expression\" to represent the handler in subscription.  An empty handler is special of case of this scenario. The handler is stilled embedded in the repository, but the handler itself it is the subscriber. So that we don't need to specified the handler at all. The subscription is like the followng\n</p>\n\n<pre data-sub=\"prettyprint:_\">\n//the handler is parameter is unnecessary here\n//because the subscriber is the handler\nhm(pathOfTheHandler).sub(publisher, \"click\"); \n</pre>\n\n<p>\nThe following is an example:\n</p>\n<pre data-sub=\"prettyprint:_;preview:_|http://jsbin.com/eposom/1\">\n\n\nhm(\"demo\", {\n  handlers: {\n    submit: function (e) {\n      hm.log(this == e.subscriber);\n      hm.log(this.path);\n      hm.log(this.raw() == e.workflow.get);\n    } \n  }\n});\n\nhm(\"demo.handlers.submit\").sub($(\"#btnSubmit\"), \"click\");\n</pre>",
        "signatures": []
    },
    {
        "name": "Activity",
    ...
    },
    {
        "name": "get",
        "type": "",
        "namespace": "Core.Unified Subscription.Subscription.Activity",
        "shortDesc": "get",
                ...
                "overloads": [
                    {
                        ...
                        "parameters": [
                            ...
                            },
                            {
                                "name": "publisher",
                                "type": "string",
                                "desc": "A publisher can be node or jQuery object. If publisher is a node, this parameter is sub-path relative to its default path of the element. The full path is their merged path. The path logic is <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Model.Path\">here</a>. If publisher is jQuery selector, then this parameter should begin with \"$\". "
                            },
                            {
                                "name": "handler",
                                "type": "string",
                                "desc": "Handler is string expression handler, the syntax of the expression is <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Handler.string\">here</a>"
                            },
                            {
                                "name": "options",
                                "type": "string",
                                "desc": "This parameter is optional. It is useful, when the handler is a workflow type and the workflow type has initialize method to handle the options. The workflow type details is <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Event.Event+Handling.Handler.workflow+type\">here</a>. You can explicitly set to undefined by using \"_\". If you have delegate behind, you can use syntax like : !events:publisher|handler|_|delgate. Otherwise, you can use !events:publisher|handler|_ or !events:publisher|handler."
                            },
                            {
                                "name": "delegate",
                                "type": "string",
                ...
                "overloads": [
                    {
                        ...
                        "parameters": [
                            ...
                            },
                            {
                                "name": "subscriber",
                                "type": "string",
                                "desc": "A subscriber can be node or jQuery object. If subscriber is a node, this parameter is sub-path relative to its default path of the element. The full path is their merged path. The path logic is <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Model.Path\">here</a>. If subscriber is jQuery selector, then this parameter should begin with \"$\"."
                            },
                            {
                                "name": "handler",
                                "type": "string",
                                "desc": "optional, string expression handler, the syntax of the expression is <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Handler.string\">here</a>.  This parameter is optional, if the subscriber is repository node, and the node value is a function handler. In this case, the function is the handler."
                            },
                            {
                                "name": "options",
                                "type": "string",
                                "desc": "optional. It is useful, when the handler is a workflow type and the workflow type has initialize method to handle the options. The workflow type details is <a href=\"fredyang/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Event.Event+Handling.Handler.workflow+type\">here</a>. You can explicitly set to undefined by using \"_\". If you have delegate behind, you can use syntax like : $events:subscriber|handler|_|delegate. Otherwise, you can use $events:subscriber|handler|_ or $events:subscriber|handler"
                            },
                            {
                                "name": "delegate",
                                "type": "",
                ...
                "overloads": [
                    {
                        ...
                        "parameters": [
                            ...
                            },
                            {
                                "name": "namespace",
                                "type": "string",
                                "desc": "namespace is sub-path relative to the namespace of parent group."
                            },
                            {
                                "name": "options",
                                "type": "string",
    ...
    },
    {
        ...
        "longDesc": "<p>\nThere two template workflow type, \"renderContent\" and \"renderSelf\".  They both use <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.templateOptions\">initialize.templateOptions</a>, <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.dataToMarkup\">convert.dataToMarkup</a>, and <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.parseSub\">finalize.parseSub</a>. You can easily compose a new template workflow with them and  your own \"get\", \"set\" activities, like the following one.\n</p>\n\n<pre data-sub=\"prettyprint:_\">\nhm.workflowType(\"renderContent\",  {\n  initialize: \"*templateOptions\",\n  get: \"get\", //extensible\n  convert: \"*dataToMarkup\",\n  set: \"html\", //extensible\n  finalize: \"*parseSub\"\n});\n</pre>\n<p>\nTo save your a few keystroke, you can use \n</p>\n<pre data-sub=\"prettyprint:_\">\nhm.workflowType(\"renderContent\",  hm.template.newTemplateWorkflow ( \"get\", \"set\" );\n</pre>",
        "signatures": []
    },
    {
    ...
    },
    {
        ...
        "longDesc": "<p>This workflow is defined as below.</p>\n\n<pre data-sub=\"prettyprint:_\">\nhm.workflowType(\"renderContent\",  {\n  initialize: \"*templateOptions\",\n  get: \"get\", //extensible\n  convert: \"*dataToMarkup\",\n  set: \"html\", //extensible\n  finalize: \"*parseSub\"\n});\n</pre>\n\n<p>\nThe activities of <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.templateOptions\">templateOptions</a> , <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.dataToMarkup\">dataToMarkup</a>, <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Template.activities.parseSub\">parseSub</a> are shared by all template workflow type. What is special here, is that , the worklow's get activity, simply just get the value of publisher, and use jQuery html method to fill the content of subscriber, which here is an html element.\n</p>\n\n<pre data-sub=\"prettyprint:_;preview:_|http://jsbin.com/ejeten/1\">\n  &lt;table&gt;\n    &lt;tr&gt;\n      &lt;td&gt;First Name&lt;/td&gt;\n      &lt;td&gt;Last Name&lt;/td&gt;\n\t&lt;/tr&gt;\n    &lt;tbody data-sub=\"!init:app.contacts|*renderContent\"&gt;\n      &lt;tr&gt;\n\t\t&lt;td&gt;\n          {{:firstName}}\n\t\t&lt;/td&gt;\n\t\t&lt;td&gt;\n          {{:lastName}}\n\t\t&lt;/td&gt;\n\t&lt;/tr&gt;\n\t&lt;/tbody&gt;\n  &lt;/table&gt;\n</pre>",
        "signatures": []
    },
    {
        "name": "renderSelf",
    ...
    },
    {
        ...
        "longDesc": "<p>\nIn Harmony, subscriptions can be created programmatically or declaratively. User can declare the subscriptions between a DOM element and the other objects (view or model) by embedding a string into an attribute in the element, the default attribute is <code>data-sub</code> ( You can change this by changing the value of <code>hm.options.subsAttr</code>)\n</p>\n<p>The attribute value is serialization of <b>subscription group</b> object,  which contains the subscription data between an element and other objects.  After page load, by default, Harmony will automatically (this can be disabled like the following), parse the elements in the page and extract the attribute value and deserialize an subscription group from the value, then use object and subscription API to build the subscriptions programmatically.</p>\n\n<pre data-sub=\"prettyprint:_;preview:_|\">\n//to disable automatic declarative subscriptions, \n//change the following value\nhm.options.autoparseSub = false;\n\n//to manually parse declarative subscritptions\n$(containerOfElement).parseSub();\n//e.g\n$(\"body\").parseSub();\n</pre>\n\n<p>The reason to call the object \"Subscription group\" is that it can contains multiple subscriptions, more than that it can contained a well-known subscription group as well. Yes a group nest other groups infinitely deep. The native form of subscription group looks like the following: </p>\n\n<pre data-sub=\"prettyprint:_\">\n{\n  //the first six properties are specified in subscription text\n  ns: \"ns\",\n  sub: [{ key: \"events\",   value: \"value\"  }],\n  pub: [{ key: \"events\",  value: \"value\"  }],\n  options: \"group options\",\n  groups: [{ key: \"groupName\", value: \"value\" }],\n  text: \"the original serialized string\",\n\n  //the following are shared over the group hierarchy, and are calculated.\n\n  elemGroup: elemGroup, //the root group                                           \n  elem: elem,\n  subscriptions: [{\n    publisher: publisher,\n    eventTypes: eventTypes,\n    subscriber: subscriber,\n    handler: handler,\n    options: options,\n    delegate: delegate\n  }],\n  appendSub: function (subscriber,  publisher, eventTypes, \n   handler,  options, delegate) {},\n  prependSub: function (subscriber,  publisher,  eventTypes, \n   handler,  options, delegate) {},\n  clearSubs: function () {}\n}                       \n</pre>\n\n<p>The string value is a group of properties separated by \";\". Each property has a property name and a property value. The following is an example:</a>\n\n<pre data-sub=\"prettyprint:_\">\npropertyName1:propertyValue2;  propertyName2:PropertyValue2;  ....\n</pre>\n\n<p>There are 4 types of <b>subscription property</b>, <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Declarative.ns\n\">ns</a>, <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Declarative.!events\">!events</a>, <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Declarative.%24events\">$events</a>, and <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Declarative.common+group\">child group</a>. All these properties are optional, but an element needs to have at least one subscription property to be called <b>subscription element</b>.\n</p>\n\n<h3>Debugging Subscription group</h3>\n\n<p>Subscription group heavily use <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Model.Path\">path calculation</a> and <a href=\"/harmony/pages/doc.html#api.apiAction=view&api.selectedEntryName=Core.Unified+Subscription.Subscription.Handler.string\">string expression of handler</a>, read them before write a subscription group. \nHarmony comes with some debugging utilities. You can use them to print out a subscription group constructed  from a string. There are usually two ways to print, both require a <span class=\"code\">&lt;div data-sub=\"logPanel:_\"&gt;&lt;/div&gt;</span>. The following shows how to programmatically printout.\n</p>\n\n<pre data-sub=\"prettyprint:_;preview:_|http://jsbin.com/oritak/2\">\n\nhm.printGroup(\"$click:count|*++;!init afterUpdate:color|css*background-color;text:firstName\");\n</pre>\n\n<p>\nIf you have subscription element, you can add a group <span class=\"code\">\"debug:_\"</span> to the group text of the element, the group object will be print out to the logPanel. The following is an example.\n</p>\n\n<pre data-sub=\"prettyprint:_;preview:_|http://jsbin.com/agahoc/1\">\n  &lt;input type=\"text\" data-sub=\"val:name|keyup;debug:_\" /&gt;\n  &lt;div data-sub=\"dump:name;debug:_\"&gt;&lt;/div&gt;\n</pre>",
        "signatures": []
    },
    {
        "name": "triggerChange",
    ...
    },
    {
        ...
        "namespace": "plugins.validation",
        "shortDesc": "apply validator to model",
